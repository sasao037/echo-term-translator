...
- ユーザー報告は `POST /api/reports`（認証不要、ハニーポット＋IPレート制限あり）で受け付け、KVに `reports:list` として蓄積されます。一覧・解決・削除は `GET/PATCH/DELETE /api/admin/reports[/:id]`（要ログイン）から行います。
...
- 送信された報告は**公開画面には一切表示されません**。管理画面の「報告一覧」タブでのみ確認・対応します（内容を確認して該当データを修正し、報告は「解決済みにする」か「削除」します）。
