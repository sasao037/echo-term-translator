...
- 送信された報告は**そのままでは公開画面に表示されません**。管理画面の「報告一覧」タブでのみ確認できます（内容を確認して該当データを修正し、報告は「解決済みにする」か「削除」します）。
- スパム対策: ハニーポット（見えない入力欄。botが埋めると見かけ上成功扱いにして黙って破棄）と、同一IPからの送信回数制限（1時間あたり5件）。追加の外部サービス（CAPTCHA等）は使っていません。

### 公開ノート（管理者が確認した仕様差分の公開）

報告の「解決済み」フラグとは別に、報告ごとに **「公開する」チェック＋公開用テキスト** を管理画面で設定できます。チェックを入れて保存すると、そのポケモンの詳細パネルに「確認された仕様差分」として表示されます。

- 「解決済み」＝管理者の対応管理用の状態、「公開する」＝サイトに出すかどうか、を意図的に分離しています。重複報告や無効な報告まで公開されないようにするためです。
- 公開文は投稿されたそのままではなく、管理画面のテキストエリアで**編集してから**公開できます（初期値は投稿者のメッセージ）。
- 公開APIは `GET /api/reports/published`（認証不要）。返すのは `pokemonId` / `category` / `publicNote` のみで、投稿者のIPや元のメッセージ、報告日時などは含みません。
...
| `published-notes.ts` | 公開済みの報告ノート（`/api/reports/published`）の遅延読み込み・キャッシュ |
